...
/** V57.7: adaptive resource-art classifier. */
class ResourceTileClassifier {
    ...
    /**
     * Callers historically supplied either the level-badge centre or a point
     * shifted up/left from it. Score both anchors and keep the stronger result.
     * This prevents a coordinate-offset bug from turning real RSS tiles into
     * UNKNOWN while remaining conservative for auto-gather decisions.
     */
    ...
    private fun classifyAt(bitmap:Bitmap,cx:Int,cy:Int,sx:Float,sy:Float):Result {
        ...
        var samples=0;var warm=0;var brown=0;var neutral=0
        var blue=0;var cyan=0;var purple=0;var vivid=0;var redEvidence=0
        for(y in top..bottom)for(x in left..right){
            ...
            val hsv=FloatArray(3);Color.RGBToHSV(r,g,b,hsv)
            val h=hsv[0];val s=hsv[1];val v=hsv[2]
            if(s>.28f&&v>.25f)vivid++
            if((h<45f||h>=330f)&&s>.25f&&v>.28f)warm++
            if(h in 15f..45f&&s>.22f&&v in .20f.. .80f)brown++
            if(h in 45f..68f&&s>.30f&&v>.45f)warm++
            if(h in 175f..250f&&s>.22f&&v>.25f)blue++
            if(h in 175f..205f&&s>.30f&&v>.35f)cyan++
            if(h in 250f..330f&&s>.20f&&v>.22f)purple++
            if(s<.25f&&v in .25f.. .82f)neutral++
        }
        ...
        val scores=linkedMapOf(Type.FOOD to food,Type.TIMBER to timber,Type.STONE to stone,Type.ORE to ore,Type.GOLD to gold)
        val ranked=scores.entries.sortedByDescending{it.value}
        val best=ranked.first();val second=ranked.getOrNull(1)?.value?:0
        ...
        return if(best.value<=0||share<.28f||margin<.06f||confidence<38)Result(Type.UNKNOWN,confidence,redEvidence)
