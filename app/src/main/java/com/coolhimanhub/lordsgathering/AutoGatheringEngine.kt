...
class AutoGatheringEngine(
    ...
    fun processDetectedTargets(targets: List<RssTarget>): RssTarget? {
        // Auto-gather is deliberately conservative: never choose an occupied
        // or weakly classified target.
        val safeTargets = targets.filter {
            !it.occupied && it.confidence >= 75 && it.level in 1..5 &&
                it.type != "Other" && it.type != "Emerging"
        }
    ...
    fun performGatheringAction(target: RssTarget) {
        ...
        // FINAL SAFE BUILD: never execute blind coordinate taps.
        // Detection/selection is retained for the next validated UI-action phase.
        return
