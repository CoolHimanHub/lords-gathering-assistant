...
class GatheringAccessibilityService :
    ...
    // V13 analyzer diagnostics
    @Volatile
    private var lastTileCount = 0

    @Volatile
    private var lastInteractiveTileCount = 0
    ...
    private fun analyseScreen(
        ...
        safeStatus(
            "Scan #$thisScan\n" +
                "TILES SCANNED: " + lastTileCount +
                "\nINTERACTIVE TILES: " + lastInteractiveTileCount +
                "\nBLUE LEVEL BADGES: " + badges.size +
                "\nReading each tile..."
        )
    ...
    private fun detectBlueBadges(
        ...
        /*
         * V13: TILE-FIRST MAP ANALYZER
         *
         * Do not treat the whole map as one blue-pixel problem.
         * The map is inspected as overlapping local tiles/windows.
         * Each window is assigned a role:
         *
         *   - INTERACTIVE: contains a compact Level badge
         *   - TERRAIN: ordinary map background / joined terrain
         *   - HUD: excluded before analysis
         *
         * A Level badge is detected from its own rectangular blue/white
         * signature. Large blue/purple resource artwork, monsters and
         * joined hills therefore cannot become a badge merely because
         * they contain blue pixels.
         */
        ...
        val mapRight = min(width, 1285)
        val mapTop = min(height, 70)
        val mapBottom = min(height, 590)

        if (mapRight <= mapLeft || mapBottom <= mapTop) {
            lastTileCount = 0
            lastInteractiveTileCount = 0
            return emptyList()
        }

        // A resource node in the supplied screenshots is much smaller
        // than the large terrain formations.  96x72 is deliberately an
        // inspection window, not a claim that the game uses square tiles.
        val tileW = 96
        val tileH = 72
        val strideX = 48
        val strideY = 36
        ...
        val candidates = mutableListOf<BlueComponent>()

        fun isBadgeBlue(pixel: Int): Boolean {
            ...
            // Badge blue: saturated blue, while excluding green terrain.
            return b >= 105 && b > r * 1.12f && b > g * 0.98f &&
                (b - r) >= 28 && (mx - mn) >= 35
        }
        ...
        fun scanTile(left: Int, top: Int, right: Int, bottom: Int) {
            tileCount++

            // Find compact blue runs. We intentionally work locally so
            // separate resource nodes can never be merged across tiles.
            ...
            for (y0 in top until bottom step 2) {
                for (x0 in left until right step 2) {
                    val k0 = key(x0, y0)
                    if (!visited.add(k0)) continue
                    if (!isBadgeBlue(bitmap.getPixel(x0, y0))) continue
                    ...
                    while (head < tail) {
                        ...
                        while (i < ns.size) {
                            ...
                            if (!isBadgeBlue(bitmap.getPixel(nx, ny))) continue
                            if (tail < qx.size) {
                                qx[tail] = nx
                                qy[tail] = ny
                                tail++
                    ...
                    // A real badge is compact; resource artwork is much larger
                    // or highly irregular. Allow partially clipped badges.
                    if (area < 45 || bw < 9 || bh < 8 || bw > 46 || bh > 34) {
                        continue
                    }

                    val aspect = bw.toFloat() / bh.toFloat()
                    if (aspect < 0.45f || aspect > 3.2f) continue

                    // The white level digit must be inside/very near the blue box.
                    val cx = (minX + maxX) / 2
                    val cy = (minY + maxY) / 2
                    var white = 0
                    var samples = 0

                    val wx0 = max(left, minX - 5)
                    val wx1 = min(right - 1, maxX + 5)
                    val wy0 = max(top, minY - 5)
                    val wy1 = min(bottom - 1, maxY + 5)

                    var wy = wy0
                    while (wy <= wy1) {
                        var wx = wx0
                        while (wx <= wx1) {
                            if (isWhite(bitmap.getPixel(wx, wy))) white++
                            samples++
                            wx += 2
                        }
                        wy += 2
                    ...
                    if (white < 4 || samples == 0) continue

                    // Reject obvious HUD-like/edge candidates even if they fall
                    // inside the map crop.
                    if (cx < mapLeft || cx >= mapRight || cy < mapTop || cy >= mapBottom) continue
                    if (insideAssistantOverlayV13(cx, cy)) continue

                    interactiveCount++

                    candidates.add(
                        BlueComponent(
                            minX = max(mapLeft, minX - 2),
                            minY = max(mapTop, minY - 2),
                            maxX = min(mapRight - 1, maxX + 2),
                            maxY = min(mapBottom - 1, maxY + 2),
                            pixels = area
                        )
                    )
                }
        ...
        while (ty < mapBottom) {
            ...
            while (tx < mapRight) {
                scanTile(
                    tx,
                    ty,
                    min(mapRight, tx + tileW),
                    min(mapBottom, ty + tileH)
                )
                tx += strideX
            }
            ty += strideY
        ...
        // Overlapping inspection windows intentionally see the same physical
        // badge. Merge only by badge-center proximity, never by blue artwork.
        val result = mutableListOf<BlueComponent>()

        for (candidate in candidates) {
            val duplicate = result.indexOfFirst { old ->
                abs(old.centerX - candidate.centerX) <= 16 &&
                    abs(old.centerY - candidate.centerY) <= 16
            }
        ...
        lastInteractiveTileCount = result.size
    ...
    private fun insideAssistantOverlayV13(x: Int, y: Int): Boolean {
