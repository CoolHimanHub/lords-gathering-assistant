...
class GatheringAccessibilityService :
    ...
    private fun classifyResource(
        ...
        /*
         * V9 RESOURCE CLASSIFIER
         *
         * The important correction is that a blue level badge alone is
         * NOT evidence of a resource.  The pixels immediately around the
         * badge are tested in several small windows.  Large low-saturation
         * map areas are ignored, because they were causing ordinary terrain
         * to be reported as STONE.
         *
         * Strong saturated signatures are preferred for ORE/WOOD/FOOD/GOLD.
         * STONE is accepted only when a compact neutral-rock signature exists.
         */
        val cx = badgeX - max(18, badgeWidth)
        val cy = badgeY - max(16, badgeHeight)
        ...
        data class Sig(
            ...
            var yellow: Int = 0,
            var orange: Int = 0,
            var bluePurple: Int = 0,
            var neutralRock: Int = 0,
            var darkRock: Int = 0
        )

        val sig = Sig()

        // Tight icon area: resource artwork is normally below/left of badge.
        val left = max(0, badgeX - max(72, badgeWidth * 3))
        val right = min(bitmap.width - 1, badgeX - max(8, badgeWidth / 5))
        val top = max(0, badgeY - max(48, badgeHeight * 2))
        val bottom = min(bitmap.height - 1, badgeY + max(20, badgeHeight))

        // Inner area receives extra weight; outer area is used only as support.
        val innerLeft = max(left, badgeX - max(58, badgeWidth * 2))
        val innerRight = right
        val innerTop = max(top, badgeY - max(38, badgeHeight * 2))
        val innerBottom = min(bottom, badgeY + max(10, badgeHeight / 2))
        ...
        while (y <= bottom) {
            ...
            while (x <= right) {
                ...
                val inInner = x >= innerLeft && x <= innerRight && y >= innerTop && y <= innerBottom
                val weight = if (inInner) 2 else 1

                val badgeBlue = b >= 90 && b > r * 1.10f && b >= g * 0.92f
                val nearWhite = r >= 235 && g >= 235 && b >= 235
                val mapGreen = g > r * 1.08f && g > b * 1.04f && sat >= 22

                if (!badgeBlue && !nearWhite && !mapGreen) {
                    sig.samples += weight

                    if (g >= 82 && g > r * 1.10f && g > b * 1.06f && sat >= 42) sig.green += weight

                    if (r >= 150 && g >= 112 && b <= 125 && r >= g * 0.95f && sat >= 42) sig.yellow += weight

                    if (r >= 150 && g >= 82 && b <= 110 && r > g * 1.10f && sat >= 48) sig.orange += weight

                    // Purple/blue crystal signature. Purple is intentionally included.
                    if (sat >= 50 && b >= 75 && (b > r * 1.03f || b > g * 1.03f)) sig.bluePurple += weight

                    // Neutral stone: gray pixels with meaningful brightness and no green cast.
                    if (bright in 55..220 && sat <= 26 && abs(r - g) <= 20 && abs(g - b) <= 20) {
                        sig.neutralRock += weight
                    }

                    if (bright in 35..150 && sat <= 34 && abs(r - g) <= 25 && abs(g - b) <= 25) {
                        sig.darkRock += weight
                    }
                }
                x += 3
        ...
        if (sig.samples < 30) return "Other"

        val foodScore = sig.yellow * 5.0f
        val goldScore = sig.orange * 5.5f + sig.yellow * 0.4f
        val woodScore = sig.green * 6.0f
        val oreScore = sig.bluePurple * 7.0f
        val stoneScore = sig.neutralRock * 5.0f + sig.darkRock * 2.0f
        ...
        val second = scores.getOrNull(1)?.value ?: 0f
        val normalized = best.value / sig.samples.toFloat()

        // Require a real visual signature; do not count background as a tile.
        if (normalized < 0.30f) return "Other"

        // Ore must have a strong saturated blue/purple signal.
        if (best.key == "Ore" && sig.bluePurple < 10) return "Other"

        // Stone must have a compact neutral signature and must beat ore clearly.
        if (best.key == "Stone") {
            if (sig.neutralRock < 14) return "Other"
            if (sig.neutralRock < sig.samples * 0.10f) return "Other"
            if (sig.green > sig.neutralRock * 0.75f) return "Other"
        }

        // Avoid ambiguous classifications.
        if (second > 0f && second / best.value > 0.78f) return "Other"

        if (best.key == "Gold" && sig.yellow > sig.orange * 2) return "Food"
    ...
    private fun resourceConfidence(
        ...
        while (y <= bottom) {
            ...
            while (x <= right) {
                ...
                if (!badgeBlue && !nearWhite && !mapGreen) {
                    total++
                    val match = when (type) {
                        "Wood" -> g >= 82 && g > r * 1.10f && g > b * 1.06f && sat >= 42
                        "Food" -> r >= 150 && g >= 112 && b <= 125 && r >= g * 0.95f && sat >= 42
                        "Gold" -> r >= 150 && g >= 82 && b <= 110 && r > g * 1.10f && sat >= 48
                        "Stone" -> bright in 55..220 && sat <= 26 && abs(r - g) <= 20 && abs(g - b) <= 20
                        "Ore" -> sat >= 50 && b >= 75 && (b > r * 1.03f || b > g * 1.03f)
                        else -> false
                    }
                    if (match) matches++
                }
        ...
        return (matches * 100 / total).coerceIn(0, 100)
    }
