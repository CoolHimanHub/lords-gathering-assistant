...
class GatheringAccessibilityService :
    ...
    private fun readLevelFromBadgeFallback(
        ...
        val left = max(0, badge.minX - 2)
        val top = max(0, badge.minY - 2)
        val right = min(bitmap.width, badge.maxX + 3)
        val bottom = min(bitmap.height, badge.maxY + 3)
        ...
        try {
            ...
            // =========================================================
            // PASS 2 - WHITE DIGIT MASK
            // =========================================================

            val whiteMask =
                createWhiteDigitMask(crop)
            ...
            // =========================================================
            // PASS 3 - BLACK DIGIT ON WHITE
            // =========================================================

            val blackMask =
                createBlackDigitMask(crop)
            ...
            // =========================================================
            // PASS 4 - BRIGHT DIGIT MASK
            // =========================================================

            val brightMask =
                createBrightDigitMask(crop)
        ...
        // At least TWO agreeing OCR passes are required
        // unless all available passes agree.
        if (best.value < 2 && votes.size >= 2) {
            return 0
        }


        // Never guess when the two strongest answers
        // have equal votes.
        if (
            second != null &&
        ...
        for (block in visionText.textBlocks) {
            for (line in block.lines) {
                for (element in line.elements) {
                    val value =
                        normalizeLevelText(element.text)
    ...
    private fun createBlackDigitMask(
        source: Bitmap
    ): Bitmap {
        ...
        while (y < source.height) {
            ...
            while (x < source.width) {
                ...
                // White digit becomes black. Everything else becomes white.
                out.setPixel(
                    x,
                    y,
                    if (isWhite) {
