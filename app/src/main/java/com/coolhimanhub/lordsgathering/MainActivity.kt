...
class MainActivity : Activity() {
    ...
    private fun buildScreen() {
        ...
        val serviceButton = Button(this).apply {
            text = "ENABLE / MANAGE ACCESSIBILITY SERVICE"
            setOnClickListener {
        ...
        val upButton = Button(this).apply {
            text = "▲ UP"
            setOnClickListener { moveSelected(-1) }
        }
        buttons.addView(upButton, LinearLayout.LayoutParams(0, dp(55), 1f))

        val downButton = Button(this).apply {
            text = "▼ DOWN"
            setOnClickListener { moveSelected(1) }
        }
        buttons.addView(downButton, LinearLayout.LayoutParams(0, dp(55), 1f))
    ...
    private fun moveSelected(direction: Int) {
        ...
        if (position == ListView.INVALID_POSITION) {
            Toast.makeText(this, "Select an RSS first", Toast.LENGTH_SHORT).show()
            return
        }

        val newPosition = position + direction
        if (newPosition < 0 || newPosition >= rssList.size) return
