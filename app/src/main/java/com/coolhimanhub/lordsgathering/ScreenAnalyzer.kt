...
class ScreenAnalyzer {
    ...
    companion object {
        ...
        private const val REF_MAP_LEFT=410f; private const val REF_MAP_RIGHT=1420f; private const val REF_MAP_TOP=90f; private const val REF_BOTTOM_MARGIN=42f
        private const val BLUE_MIN=48; private const val BLUE_DELTA_R=12; private const val BLUE_DELTA_G=2; private const val RED_MAX=195
        private const val GLYPH_MIN_RGB=125; private const val GLYPH_MAX_SAT=165; private const val GLYPH_MIN_PIXELS=3; private const val LEVEL_OCR_TIMEOUT_MS=180L
    }
    ...
    fun analyzeScreenshot(bitmap:Bitmap,expectedRegionX:IntRange=0 until bitmap.width,expectedRegionY:IntRange=0 until bitmap.height,ignoredRegions:List<BoundingBox> = emptyList()):List<RssDetection>{
        ...
        val raw=findBadges(bitmap,left,right,top,bottom,sx,sy,ignoredRegions).map{badge->
            val probeX=(badge.box.centerX-(18f*sx).toInt()).coerceIn(left+8,right-8); val probeY=(badge.box.centerY-(7f*sy).toInt()).coerceIn(top+8,bottom-8)
            val visual=resourceClassifier.classify(bitmap,probeX,probeY)
            val type=when(visual.type){ResourceTileClassifier.Type.FOOD->"food";ResourceTileClassifier.Type.TIMBER->"timber";ResourceTileClassifier.Type.STONE->"stone";ResourceTileClassifier.Type.ORE->"ore";ResourceTileClassifier.Type.GOLD->"gold";ResourceTileClassifier.Type.UNKNOWN->"unknown"}
            val level=readBadgeLevel(bitmap,badge.box,sx,sy); val classifiedConfidence=if(visual.type==ResourceTileClassifier.Type.UNKNOWN)0 else min(badge.confidence,visual.confidence)
            val finalConfidence=when{level in 1..5&&classifiedConfidence>=45->min(95,classifiedConfidence+8);level in 1..5->min(90,badge.confidence);else->classifiedConfidence}
            RssDetection(type,level,probeX,probeY,badge.box,finalConfidence,false,visual.redEvidence)
        }
        val result=gridPipeline.normalize(raw).map{it.detection}
        val typeCounts=result.groupingBy{it.type}.eachCount(); val known=result.count{it.type!="unknown"}; val leveled=result.count{it.level in 1..5}; val candidateCount=result.count{it.confidence>=68&&it.type!="unknown"&&it.level in 1..5}; val unknownCount=result.count{it.type=="unknown"}
        Log.i("LordsAssistantDiag","raw=${raw.size} grid=${result.size} known=$known leveled=$leveled candidates=$candidateCount types=$typeCounts unknown=$unknownCount")
    ...
    private fun readBadgeLevel(bitmap:Bitmap,box:BoundingBox,sx:Float,sy:Float):Int{
        val padX=max(3,(5f*sx).toInt()); val padY=max(3,(5f*sy).toInt()); val l=(box.minX-padX).coerceAtLeast(0); val t=(box.minY-padY).coerceAtLeast(0); val r=(box.maxX+padX).coerceAtMost(bitmap.width-1); val b=(box.maxY+padY).coerceAtMost(bitmap.height-1); if(r<=l||b<=t)return 0
    ...
    /** Two-pass candidate discovery. Pass 1 is strict; pass 2 catches dim/anti-aliased badges. Both are deduplicated before classification. */
    private fun findBadges(bitmap:Bitmap,left:Int,right:Int,top:Int,bottom:Int,sx:Float,sy:Float,ignoredRegions:List<BoundingBox>):List<Badge>{
        val strict=collectBadges(bitmap,left,right,top,bottom,sx,sy,ignoredRegions,false)
        val relaxed=collectBadges(bitmap,left,right,top,bottom,sx,sy,ignoredRegions,true)
        val all=(strict+relaxed).sortedWith(compareByDescending<Badge>{it.confidence}.thenBy{it.box.centerY}.thenBy{it.box.centerX})
        val out=ArrayList<Badge>(); val mergeX=max(16,(16f*sx).toInt()); val mergeY=max(15,(15f*sy).toInt())
        for(b in all)if(out.none{e->abs(e.box.centerX-b.box.centerX)<mergeX&&abs(e.box.centerY-b.box.centerY)<mergeY})out+=b
    ...
    private fun collectBadges(bitmap:Bitmap,left:Int,right:Int,top:Int,bottom:Int,sx:Float,sy:Float,ignoredRegions:List<BoundingBox>,relaxed:Boolean):List<Badge>{
        val width=right-left+1; val height=bottom-top+1; val size=width*height; val pixels=IntArray(size); bitmap.getPixels(pixels,0,width,left,top)
        ...
        for(y in 0 until height)for(x in 0 until width){val start=y*width+x; if(visited[start]||!blue[start])continue; var head=0; var tail=0; queue[tail++]=start; visited[start]=true; var minX=x; var maxX=x; var minY=y; var maxY=y; var area=0
            ...
            val bw=maxX-minX+1; val bh=maxY-minY+1; if(bw !in minW..maxW||bh !in minH..maxH||area !in minArea..maxArea)continue; val density=area.toFloat()/(bw*bh).toFloat(); if(density<if(relaxed).07f else .10f||density>.97f)continue
            var light=0; for(gy in minY..maxY)for(gx in minX..maxX){val c=pixels[gy*width+gx]; val r=Color.red(c); val g=Color.green(c); val b=Color.blue(c); val mx=max(r,max(g,b)); val mn=min(r,min(g,b)); if(r>=GLYPH_MIN_RGB&&g>=GLYPH_MIN_RGB&&b>=GLYPH_MIN_RGB&&mx-mn<=GLYPH_MAX_SAT)light++}
            if(light<if(relaxed)2 else GLYPH_MIN_PIXELS)continue
            val ax1=left+minX; val ay1=top+minY; val ax2=left+maxX; val ay2=top+maxY; if(ignoredRegions.any{region->ax1<=region.maxX&&ax2>=region.minX&&ay1<=region.maxY&&ay2>=region.minY})continue
            val aspect=bw.toFloat()/bh; val aspectScore=(1f-abs(aspect-1.35f)/1.9f).coerceIn(0f,1f); val densityScore=(1f-abs(density-.45f)/.60f).coerceIn(0f,1f); val glyphScore=light.coerceAtMost(80)/80f; val base=if(relaxed)58f else 64f; val confidence=(base+16f*aspectScore+12f*densityScore+8f*glyphScore).toInt().coerceIn(if(relaxed)58 else 64,96); found+=Badge(BoundingBox(ax1,ay1,ax2,ay2),confidence)
        }
        return found
    }
