...
/**
 * V21 - conservative RSS badge detector.
 *
 * Design goal: fewer false positives, not maximum recall.
 * A target is accepted only when a compact blue level badge is paired with
 * a plausible resource-art zone immediately to its left/up-left.
 * HUD, monsters, water and large terrain blobs are rejected by geometry and
 * local colour signatures. Coordinates returned are SCREEN PIXELS.
 *
 * V21 remains observation-only at the detector level. Troop sending is not
 * performed here.
 */
class ScreenAnalyzer {
    ...
    fun analyzeScreenshot(
        ...
        for (badge in badges) {
            ...
            val occupied = flagScore(bitmap, badge.box, left, top, right, bottom) >= 58
            val local = localQuality(bitmap, badge.box, art.box)
            var confidence = (badge.blueRatio * 100f * 0.28f + art.confidence * 0.56f + local * 0.16f).toInt()
    ...
    private fun findBadges(bitmap: Bitmap, left: Int, right: Int, top: Int, bottom: Int): List<Badge> {
        ...
        fun blue(x: Int, y: Int): Boolean {
            ...
            return b >= 82 && b - r >= 16 && b >= g * 0.94f && b >= r * 1.08f
        }
        ...
        for (gy in 0 until gh) for (gx in 0 until gw) {
            ...
            if (!blue(sx, sy)) { visited[idx] = true; continue }

            q.clear(); q.add(idx); visited[idx] = true
            var minGX = gx; var maxGX = gx; var minGY = gy; var maxGY = gy; var count = 0
            while (q.isNotEmpty()) {
                val p = q.removeFirst(); val py = p / gw; val px = p % gw; count++
                ...
                for (dy in -1..1) for (dx in -1..1) {
                    ...
                    if (blue(xx, yy)) { visited[ni] = true; q.add(ni) }
                }
            }
    ...
    private fun readBadgeDigit(bitmap: Bitmap, box: BoundingBox): Int? {
        // The game badge contains a small white 1-5 glyph. We use shape mass
        // and only accept a digit when it is clearly separated from the badge.
        ...
        for (y in max(box.minY, cy - 12)..min(box.maxY, cy + 12)) {
            for (x in max(box.minX, cx - 12)..min(box.maxX, cx + 12)) {
                val c = bitmap.getPixel(x, y)
                val r = Color.red(c); val g = Color.green(c); val b = Color.blue(c)
                if (r >= 155 && g >= 155 && b >= 155 && max(r, max(g, b)) - min(r, min(g, b)) < 110) {
                    samples += x to y
                }
            }
        }
        if (samples.size < 12) return null
        val x0 = samples.minOf { it.first }; val x1 = samples.maxOf { it.first }
        val y0 = samples.minOf { it.second }; val y1 = samples.maxOf { it.second }
        ...
        for (gy in 0 until 7) for (gx in 0 until 5) {
            val xa = x0 + gx * w / 5; val xb = max(xa + 1, x0 + (gx + 1) * w / 5)
            val ya = y0 + gy * h / 7; val yb = max(ya + 1, y0 + (gy + 1) * h / 7)
            var on = 0; var total = 0
            for (yy in ya until min(y0 + h, yb)) for (xx in xa until min(x0 + w, xb)) {
                total++; val c = bitmap.getPixel(xx, yy)
                val r = Color.red(c); val g = Color.green(c); val b = Color.blue(c)
                if (r >= 155 && g >= 155 && b >= 155 && max(r, max(g, b)) - min(r, min(g, b)) < 110) on++
            }
        ...
        if (scores[0].second < 16f || scores[0].second - scores[1].second < 3.5f) return null
        return scores[0].first
    }

    private data class Art(val type: String, val confidence: Int, val box: BoundingBox, val dominant: Int)

    private fun classifyArtwork(bitmap: Bitmap, badge: BoundingBox, left: Int, top: Int, right: Int, bottom: Int): Art? {
        val l = max(left, badge.minX - 82); val r = min(right, badge.minX - 3)
        val t = max(top, badge.centerY - 48); val b = min(bottom, badge.maxY + 20)
        if (r <= l || b <= t) return null

        var blue = 0; var cyan = 0; var warm = 0; var yellow = 0; var gray = 0; var total = 0
        var minX = r; var minY = b; var maxX = l; var maxY = t
        ...
        for (y in t..b step 2) for (x in l..r step 2) {
            ...
            val mx = max(rr, max(gg, bb)); val mn = min(rr, min(gg, bb)); val ch = mx - mn
            total++; sumR += rr; sumG += gg; sumB += bb
            ...
            if (ch > 20 && (rr > 70 || gg > 70 || bb > 70)) { minX=min(minX,x); maxX=max(maxX,x); minY=min(minY,y); maxY=max(maxY,y) }
        }
        if (total < 30 || maxX <= minX || maxY <= minY) return null
        val cyanR = cyan.toFloat()/total; val warmR=warm.toFloat()/total; val yellowR=yellow.toFloat()/total; val grayR=gray.toFloat()/total; val blueR=blue.toFloat()/total
        ...
        val dominant=Color.rgb(sumR/total,sumG/total,sumB/total)
        return Art(best.first, conf, BoundingBox(minX,minY,maxX,maxY), dominant)
    }

    private fun flagScore(bitmap: Bitmap, badge: BoundingBox, left: Int, top: Int, right: Int, bottom: Int): Int {
        val l=max(left,badge.minX-24); val r=min(right,badge.maxX+30)
        val t=max(top,badge.minY-40); val b=min(bottom,badge.minY+10)
        var red=0; var strong=0; var samples=0; var redTop=0
        for(y in t..b step 2) for(x in l..r step 2){
            val c=bitmap.getPixel(x,y); val rr=Color.red(c); val gg=Color.green(c); val bb=Color.blue(c); samples++
            if(rr>=150 && rr>gg*1.35f && rr>bb*1.25f){ red++; if(rr>=190&&rr>gg*1.45f&&rr>bb*1.35f) strong++; if(y<badge.centerY-8) redTop++ }
        }
        if(samples==0) return 0
        var score=0; if(red>=5)score+=18; if(red>=12)score+=18; if(strong>=3)score+=12; if(redTop>=3)score+=15; if(red>100)score-=20
        return score.coerceIn(0,100)
    }

    private fun localQuality(bitmap: Bitmap, badge: BoundingBox, art: BoundingBox): Int {
    ...
    private fun blueRatio(bitmap: Bitmap, box: BoundingBox): Float {
        var n=0; var b=0
        for(y in box.minY..box.maxY step 2) for(x in box.minX..box.maxX step 2){ n++; val c=bitmap.getPixel(x,y); val r=Color.red(c); val g=Color.green(c); val bb=Color.blue(c); if(bb>=82&&bb-r>=16&&bb>=g*.94f) b++ }
        return if(n==0)0f else b.toFloat()/n
    }

    private fun insideLikelyHud(x:Int,y:Int):Boolean = y < 72 || x < 105 || x > 1290 || y > 600

    private fun dedupe(input: List<RssDetection>): List<RssDetection> {
        val out=ArrayList<RssDetection>()
        for(c in input){
            val i=out.indexOfFirst{abs(it.centerX-c.centerX)<38 && abs(it.centerY-c.centerY)<38}
            if(i<0) out+=c else if(c.confidence>out[i].confidence) out[i]=c
        }
        return out.sortedWith(compareByDescending<RssDetection>{it.confidence}.thenByDescending{it.level})
    }
}
