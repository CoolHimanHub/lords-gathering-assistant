...
class ScreenAnalyzer {
    ...
    fun analyzeScreenshot(bitmap:Bitmap,expectedRegionX:IntRange=0 until bitmap.width,expectedRegionY:IntRange=0 until bitmap.height,ignoredRegions:List<BoundingBox> = emptyList()):List<RssDetection>{
        ...
        val raw=findBadges(bitmap,left,right,top,bottom,sx,sy,ignoredRegions).map{badge->
            ...
            RssDetection(type,level,probeX,probeY,badge.box,finalConfidence,false,visual.redEvidence,false,0)
        }
    ...
    private fun readBadgeLevel(bitmap:Bitmap,box:BoundingBox,sx:Float,sy:Float):Int{
        val padX=max(3,(5f*sx).toInt()); val padY=max(3,(5f*sy).toInt())
        val l=(box.minX-padX).coerceAtLeast(0); val t=(box.minY-padY).coerceAtLeast(0)
        val r=(box.maxX+padX).coerceAtMost(bitmap.width-1); val b=(box.maxY+padY).coerceAtMost(bitmap.height-1)
        if(r<=l||b<=t)return 0
        return try{
            ...
            val text=try{Tasks.await(badgeRecognizer.process(InputImage.fromBitmap(scaled,0)),LEVEL_OCR_TIMEOUT_MS,TimeUnit.MILLISECONDS)?.text?:""}catch(_:Exception){""}
            try{scaled.recycle()}catch(_:Exception){}; try{crop.recycle()}catch(_:Exception){ }
            Regex("[1-5]").find(text)?.value?.toIntOrNull()?.coerceIn(1,5)?:0
        }catch(_:Exception){0}
    }
    ...
    private fun collectBadges(bitmap:Bitmap,left:Int,right:Int,top:Int,bottom:Int,sx:Float,sy:Float,ignoredRegions:List<BoundingBox>,relaxed:Boolean):List<Badge>{
        val width=right-left+1; val height=bottom-top+1; val size=width*height
        ...
        while(i<size){
            val lx=i%width; val ly=i/width; val ax=left+lx; val ay=top+ly
            if(!ignored(ax,ay)){
                val c=pixels[i]; val r=Color.red(c); val g=Color.green(c); val b=Color.blue(c)
                val minBlue=if(relaxed)40 else BLUE_MIN; val deltaR=if(relaxed)8 else BLUE_DELTA_R; val deltaG=if(relaxed)-2 else BLUE_DELTA_G
                blue[i]=b>=minBlue&&b-r>=deltaR&&b-g>=deltaG&&r<=if(relaxed)215 else RED_MAX
            }
            i++
        }
        val visited=BooleanArray(size); val queue=IntArray(size); val found=ArrayList<Badge>()
        val minW=max(7,(10f*sx).toInt()); val maxW=max(minW+2,(60f*sx).toInt())
        val minH=max(6,(9f*sy).toInt()); val maxH=max(minH+2,(46f*sy).toInt())
        val minArea=max(30,(55f*sx*sy).toInt()); val maxArea=max(minArea+1,(1500f*sx*sy).toInt())
        for(y in 0 until height)for(x in 0 until width){
            val start=y*width+x; if(visited[start]||!blue[start])continue
            ...
            while(head<tail){
                ...
                for(dy in -1..1)for(dx in -1..1)if(dx!=0||dy!=0){
                    val nx=px+dx; val ny=py+dy; if(nx !in 0 until width||ny !in 0 until height)continue
                    val ni=ny*width+nx; if(!visited[ni]&&blue[ni]){visited[ni]=true; queue[tail++]=ni}
                }
            }
            ...
            val minDensity=if(relaxed)0.07f else 0.10f
            if(density<minDensity||density>0.97f)continue
            var light=0
            for(gy in minY..maxY)for(gx in minX..maxX){
                val c=pixels[gy*width+gx]; val r=Color.red(c); val g=Color.green(c); val b=Color.blue(c)
                val mx=max(r,max(g,b)); val mn=min(r,min(g,b)); if(r>=GLYPH_MIN_RGB&&g>=GLYPH_MIN_RGB&&b>=GLYPH_MIN_RGB&&mx-mn<=GLYPH_MAX_SAT)light++
            }
            ...
            val aspect=bw.toFloat()/bh; val aspectScore=(1f-abs(aspect-1.35f)/1.9f).coerceIn(0f,1f)
            val densityScore=(1f-abs(density-.45f)/.60f).coerceIn(0f,1f); val glyphScore=light.coerceAtMost(80)/80f
            val base=if(relaxed)58f else 64f
            val confidence=(base+16f*aspectScore+12f*densityScore+8f*glyphScore).toInt().coerceIn(if(relaxed)58 else 64,96)
