...
class MainActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        root.addView(TextView(this).apply {
            text = "LM Companion  •  v0.1"; textSize = 20f; setTextColor(Color.WHITE)
            setPadding(0, 0, 0, 12)
        })
        root.addView(TextView(this).apply {
            text = "Resource + monster scanner / compact overlay"
            setTextColor(0xFFB8BBC4.toInt()); setPadding(0, 0, 0, 14)
        })
        root.addView(Switch(this).apply {
            text = "Always-on-top overlay"; setTextColor(Color.WHITE); isChecked = current.overlayEnabled
        ...
        root.addView(TextView(this).apply {
            text = "RESOURCE PREFERENCES"; setTextColor(0xFF8D91A0.toInt()); setPadding(0, 18, 0, 4)
        })
        ResourceType.values().forEach { type ->
            root.addView(CheckBox(this).apply {
                text = type.name; setTextColor(Color.WHITE)
                isChecked = current.resourceTypes.contains(type)
                setOnCheckedChangeListener { _, checked -> store.setResourceEnabled(type, checked) }
            })
        }
        val levelRow = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL }
        (1..5).forEach { level ->
            levelRow.addView(CheckBox(this).apply {
                text = "L$level"; setTextColor(Color.WHITE)
                isChecked = current.resourceLevels.contains(level)
                setOnCheckedChangeListener { _, checked -> store.setResourceLevelEnabled(level, checked) }
            })
        }
        root.addView(levelRow)
        root.addView(Button(this).apply {
            text = "Monster lineup quick reference"
            setOnClickListener { startActivity(Intent(this@MainActivity, LineupActivity::class.java)) }
        })
        root.addView(Switch(this).apply {
            text = "Automatic actions (advanced)"; setTextColor(Color.WHITE); isChecked = current.automaticActions
        ...
        root.addView(Button(this).apply {
            text = "Start screen scanner"; setOnClickListener { requestCapture() }
        })
        root.addView(TextView(this).apply {
            text = "Workflow: screen → CV → OCR → coordinate → validation → overlay"
            setTextColor(0xFFB8BBC4.toInt()); gravity = Gravity.CENTER_HORIZONTAL; setPadding(0, 20, 0, 0)
        })
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (store.load().overlayEnabled) startService(Intent(this, OverlayService::class.java))
    }
}
